    ...
    flake-utils.lib.eachDefaultSystem (system:
      ...
      in
      {
        ...
        packages = {
          ...
          #     $ nix run -- <args?>
          #
          default = self.packages.${system}.mocaml;

          mocaml = ocamlPackages.buildDunePackage {
            pname = "mocaml";
            version = "0.1.0";
            duneVersion = "3";
            src = sources.ocaml;
        ...
        #
        checks = {
          # Run tests for the `mocaml` package
          mocaml =
            let
            ...
            self.packages.${system}.mocaml.overrideAttrs
              (oldAttrs: {
                name = "check-${oldAttrs.name}";
                doCheck = true;
                buildPhase = patchDuneCommand oldAttrs.buildPhase;
                checkPhase = patchDuneCommand oldAttrs.checkPhase;
                # skip installation (this will be tested in the `mocaml-app` check)
